...
equation
  der(Td) = (Functions.sat(Kp*(Td0-Td),0, Qmax) - sum(Gh*(Td - Tu[i])*(Functions.sat(b*x[i], -0.5, 0.5)+0.5) for i in 1:N))/Cd;
  for i in 1:N loop
    der(Tu[i]) = (Gh*(Td - Tu[i])*(Functions.sat(b*x[i], -0.5, 0.5)+0.5) - Gu*(Tu[i] - (278.15 + 8*sin(2*pi*time/86400))))/Cu[i];
    der(x[i]) = a*Functions.hist(x[i], Tu0 - Tu[i], Teps);
  end for;
end HeatingSystemExplicit_model;
