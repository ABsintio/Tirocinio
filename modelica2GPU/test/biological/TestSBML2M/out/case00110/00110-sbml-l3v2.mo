...
model case00110 "00110-sbml-l3v2"
...
equation
    ...
    der(S1_amount) =  - (compartment * S2 * multiply(k1, S1));
    der(S2_amount) = (2.0 * compartment * S2 * multiply(k1, S1)) - (compartment * S2 * multiply(k1, S1)) - (compartment1 * k2 * (S2 -1 * S3));
    der(S3_amount) = (compartment1 * k2 * (S2 -1 * S3)) ;
    S1 = S1_conc;
    S2 = S2_conc;
    S3 = S3_conc;
