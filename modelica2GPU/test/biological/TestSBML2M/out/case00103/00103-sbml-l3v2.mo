...
model case00103 "00103-sbml-l3v2"
...
equation
    ...
    der(S1_amount) = (k2 * S3 * S4) - (multiply(k1, multiply(S1, S2)));
    der(S2_amount) = (k2 * S3 * S4) - (multiply(k1, multiply(S1, S2)));
    der(S3_amount) = (multiply(k1, multiply(S1, S2))) - (k2 * S3 * S4);
    der(S4_amount) = (multiply(k1, multiply(S1, S2))) - (k2 * S3 * S4);
    S1 = S1_conc;
    S2 = S2_conc;
    S3 = S3_conc;
    S4 = S4_conc;
