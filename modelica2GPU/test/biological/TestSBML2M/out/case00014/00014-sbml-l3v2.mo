...
model case00014 "00014-sbml-l3v2"
...
equation
    ...
    der(S1_amount) = (2.0 * compartment * k2 * S3) - (2.0 * compartment * k1 * pow(S1, 2) * S2);
    der(S2_amount) = (compartment * k2 * S3) - (compartment * k1 * pow(S1, 2) * S2);
    der(S3_amount) = (compartment * k1 * pow(S1, 2) * S2) - (compartment * k2 * S3);
    S1 = S1_conc;
    S2 = S2_conc;
    S3 = S3_conc;
