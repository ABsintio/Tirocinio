...
model BIOMD420 "Ratushny2012_ASSURE_I"
...
equation
    dsp1ksp = Ksp / 2 * (1 + (s + P1) / Ksp - pow(pow(1 + (s + P1) / Ksp, 2) - 4 * s * P1 / pow(Ksp, 2), 0.5));
    dsp1p2kd = Kd / 2 * (1 + (dsp1ksp + P2 * univ) / Kd - pow(pow(1 + (dsp1ksp + P2 * univ) / Kd, 2) - 4 * dsp1ksp * P2 * univ / pow(Kd, 2), 0.5));
    der(P2) = (ks * (k0 + pow(dsp1p2kd / ka, h)) / (1 + pow(dsp1p2kd / ka, h))) - (__RATE___1 * P2);
