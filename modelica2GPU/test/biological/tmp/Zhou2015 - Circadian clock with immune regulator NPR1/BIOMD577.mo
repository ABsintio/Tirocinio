...
model BIOMD577 "Zhou2015 - Circadian clock with immune regulator NPR1"
	...
	input Real F_PRR7;
        output Real y;
    algorithm
        y :=  (F_PRR7 * parameter_6^parameter_7 / (parameter_6^parameter_7 + cT^parameter_7)) * (n8 * (cLm + cL)^e / ((cLm + cL)^e + g10^e) + n9 * cP9^f / (cP9^f + g11^f)) / def;
	...
	input Real F_TOC1;
        output Real y;
    algorithm
        y :=  (F_TOC1 * n2 / (1 + (cL / (g5 * (1 + (species_3 / parameter_14)^parameter_11)))^e)) * g4 / (cEC + g4) / def;
	...
	input Real F_LHY;
        output Real y;
    algorithm
        y :=  F_LHY * (L * q1 * cP + n1 * g1^a / ((cP9 + cP7 + cNI + cT)^a + g1^a));
    ...
    parameter Real Kd_PRR7 = 0.0;
    parameter Real default = 1.0;
    parameter Real def = 1.0;
    parameter Real compartment_1 = 1.0;

    Real D(start=0.0);
    Real nb_TOC1(start=0.5606);
    Real nb_LHY(start=0.4808);
    Real nb_PRR7(start=0.3918);
    Real na_TOC1(start=1.02692174);
    Real na_LHY(start=1.82041904);
    Real na_PRR7(start=0.6082);
    Real Kd_TOC1(start=1.3371);
    Real Kd_LHY(start=2.5062);
    Real NPR1_WT(start=1.0);
    Real NPR1_SA(start=1.0);
    Real PRR7_on(start=1.0);
    Real LHY_on(start=1.0);
    Real WT(start=1.0);
    Real SA(start=1.0);
    Real F_TOC1(start=1.0);
    Real F_LHY(start=1.0);
    Real F_PRR7(start=1.0);
...
equation
    D = 1 - L;
    NPR1_WT = piecewise(1, lt(time, 28), piecewise(-0.0113 * (time - 28 - floor((time - 28) / 24) * 24) + 0.6286, lt(piecewise(time - 28 - 24 * ceil((time - 28) / 24), xor(lt(time - 28, 0), lt(24, 0)), time - 28 - 24 * floor((time - 28) / 24)), 4), piecewise(0.003 * (time - 28 - floor((time - 28) / 24) * 24) + 0.5716, and(geq(piecewise(time - 28 - 24 * ceil((time - 28) / 24), xor(lt(time - 28, 0), lt(24, 0)), time - 28 - 24 * floor((time - 28) / 24)), 4), lt(piecewise(time - 28 - 24 * ceil((time - 28) / 24), xor(lt(time - 28, 0), lt(24, 0)), time - 28 - 24 * floor((time - 28) / 24)), 8)), piecewise(0.0774 * (time - 28 - floor((time - 28) / 24) * 24) - 0.0232, and(geq(piecewise(time - 28 - 24 * ceil((time - 28) / 24), xor(lt(time - 28, 0), lt(24, 0)), time - 28 - 24 * floor((time - 28) / 24)), 8), lt(piecewise(time - 28 - 24 * ceil((time - 28) / 24), xor(lt(time - 28, 0), lt(24, 0)), time - 28 - 24 * floor((time - 28) / 24)), 12)), piecewise(0.1815 * (time - 28 - floor((time - 28) / 24) * 24) - 1.2732, and(geq(piecewise(time - 28 - 24 * ceil((time - 28) / 24), xor(lt(time - 28, 0), lt(24, 0)), time - 28 - 24 * floor((time - 28) / 24)), 12), lt(piecewise(time - 28 - 24 * ceil((time - 28) / 24), xor(lt(time - 28, 0), lt(24, 0)), time - 28 - 24 * floor((time - 28) / 24)), 16)), piecewise(0.0085 * (time - 28 - floor((time - 28) / 24) * 24) + 1.4947, and(geq(piecewise(time - 28 - 24 * ceil((time - 28) / 24), xor(lt(time - 28, 0), lt(24, 0)), time - 28 - 24 * floor((time - 28) / 24)), 16), lt(piecewise(time - 28 - 24 * ceil((time - 28) / 24), xor(lt(time - 28, 0), lt(24, 0)), time - 28 - 24 * floor((time - 28) / 24)), 20)), piecewise(-0.2591 * (time - 28 - floor((time - 28) / 24) * 24) + 6.8481, and(geq(piecewise(time - 28 - 24 * ceil((time - 28) / 24), xor(lt(time - 28, 0), lt(24, 0)), time - 28 - 24 * floor((time - 28) / 24)), 20), lt(piecewise(time - 28 - 24 * ceil((time - 28) / 24), xor(lt(time - 28, 0), lt(24, 0)), time - 28 - 24 * floor((time - 28) / 24)), 24)), 0)))))));
    NPR1_SA = piecewise(piecewise(1.2393 * (time - 28) + 0.6286, and(gt(time, 28), leq(time, 32)), piecewise(0.5562 * (time - 28) + 3.3613, and(gt(time, 32), leq(time, 36)), piecewise(0.5897 * (time - 28) + 3.093, and(gt(time, 36), leq(time, 40)), piecewise(1.1649 * (time - 28) - 3.8089, and(gt(time, 40), leq(time, 44)), piecewise(0.6696 * (time - 28) + 4.1157, and(gt(time, 44), leq(time, 48)), piecewise(-0.8328 * (time - 28) + 34.1643, and(gt(time, 48), leq(time, 52)), piecewise(-0.4128 * (time - 28) + 24.0839, and(gt(time, 52), leq(time, 56)), piecewise(-1.7801 * (time - 28) + 62.3676, and(gt(time, 56), leq(time, 60)), piecewise(-0.2292 * (time - 28) + 12.7375, and(gt(time, 60), leq(time, 64)), piecewise(1.2106 * (time - 28) - 39.0947, and(gt(time, 64), leq(time, 68)), piecewise(-0.6097 * (time - 28) + 33.7203, and(gt(time, 68), leq(time, 72)), piecewise(-1.5657 * (time - 28) + 75.7827, and(gt(time, 72), leq(time, 76)), 0)))))))))))), and(gt(time, 28), lt(time, 76)), NPR1_WT);
    PRR7_on = piecewise(1, true, 0);
    LHY_on = 1;
    nb_TOC1 = piecewise(0.5606, and(eq(LHY_on, 1), eq(PRR7_on, 1)), piecewise(0.5782, eq(LHY_on, 1), piecewise(0.5502, eq(PRR7_on, 1), 0.5689)));
    nb_LHY = piecewise(0.4808, and(eq(LHY_on, 1), eq(PRR7_on, 1)), piecewise(0.3646, eq(LHY_on, 1), 1));
    nb_PRR7 = piecewise(0.3918, and(eq(LHY_on, 1), eq(PRR7_on, 1)), piecewise(0.2113, eq(PRR7_on, 1), 1));
    Kd_TOC1 = piecewise(1.3371, and(eq(LHY_on, 1), eq(PRR7_on, 1)), piecewise(1.3925, eq(LHY_on, 1), piecewise(1.0212, eq(PRR7_on, 1), 1.0714)));
    Kd_LHY = piecewise(2.5062, and(eq(LHY_on, 1), eq(PRR7_on, 1)), piecewise(1.9185, eq(LHY_on, 1), 0));
    WT = piecewise(1, true, 0);
    na_TOC1 = piecewise((1 - nb_TOC1) * (1 + Kd_TOC1), eq(WT, 1), 0);
    na_LHY = piecewise((1 - nb_LHY) * (1 + Kd_LHY), eq(WT, 1), 0);
    na_PRR7 = piecewise((1 - nb_PRR7) * (1 + Kd_PRR7), eq(WT, 1), 0);
    SA = piecewise(1, true, 0);
    F_TOC1 = piecewise(nb_TOC1 + na_TOC1 * NPR1_SA / (Kd_TOC1 + NPR1_SA), eq(SA, 1), nb_TOC1 + na_TOC1 * NPR1_WT / (Kd_TOC1 + NPR1_WT));
    F_LHY = piecewise(nb_LHY + na_LHY * NPR1_SA / (Kd_LHY + NPR1_SA), eq(SA, 1), nb_LHY + na_LHY * NPR1_WT / (Kd_LHY + NPR1_WT));
    F_PRR7 = piecewise(nb_PRR7 + na_PRR7 * NPR1_SA / (Kd_PRR7 + NPR1_SA), eq(SA, 1), nb_PRR7 + na_PRR7 * NPR1_WT / (Kd_PRR7 + NPR1_WT));
    ...
    der(cL_m) = (def * function_4_cL_m_trscr(L, a, cNI, cP, cP7, cP9, cT, g1, n1, q1, F_LHY)) - (def * function_4_cL_m_degr(L, cL_m, m1, m2));
    ...
    der(cP7_m) = (def * function_4_cP7_m_trscr_1(cL, cLm, cP9, cT, def, e, f, g10, g11, n8, n9, parameter_6, parameter_7, F_PRR7)) - (def * function_4_cP7_m_degr(cP7_m, def, m14));
    ...
    der(cT_m) = (def * function_4_cT_m_trscr(cEC, cL, def, e, g4, g5, n2, parameter_11, parameter_14, species_3, F_TOC1)) - (def * function_4_cT_m_degr(cT_m, def, m5));
