...
#ifndef BIOMD449_PERTHREAD_SYSTEMDEFINITION_H
#define BIOMD449_PERTHREAD_SYSTEMDEFINITION_H
...
template<class Precision> __forceinline__ __device__ void PerThread_OdeFunction(
    ...
    ACC[11]=X[26];
    ACC[9]=(0.1149 * X[20]);
    ACC[8]=(0.7465000000000001 * X[19]);
    ACC[10]=X[24];
    ACC[1]=(0.026656 * X[1]);
    ACC[7]=(0.013 * (X[16] + X[15]));
    ACC[6]=(0.04356 * (X[14] + X[15]));
    ACC[2]=(0.05866 * (X[8] + X[6]));
    ACC[3]=(0.3761 * (X[7] + X[8]));
    ACC[4]=(X[9] + X[11]);
    ACC[5]=(5.202 * (X[12] + X[11]));
    ...
    F[8]=((863.85 * (X[7] * X[24])) + (-283.713 * X[8]));
    F[7]=((3.227 * (X[5] * X[11])) + ((280.8 * X[8]) + (X[7] * (-3424.0 + (-863.85 * X[24])))));
...
template<class Precision> __forceinline__ __device__ void PerThread_Initialization(
    ...
    X[0]=95.4699007486273; //AS160
    X[1]=0.0; //AS160p
    X[2]=45.4119141649757; //GLUT4
    X[3]=-9.723833195249931; //GLUT4m
    X[4]=49.9344643421136; //IR
    X[5]=86.2418960059256; //IRS1
    X[6]=13.7482359094757; //IRS1307
    X[7]=0.00095272377217019; //IRS1p
    X[8]=0.008915310755769471; //IRS1p307
    X[9]=0.0527894351383809; //IRi
    X[10]=0.0; //IRins
    X[11]=0.011815001204792; //IRip
    X[12]=0.000931221500588088; //IRp
    X[13]=78.02193221158591; //PKB
    X[14]=12.2197372437326; //PKB308p
    X[15]=0.061992532897245; //PKB308p473p
    X[16]=9.696338994578401; //PKB473p
    X[17]=98.2037625421647; //S6
    X[18]=99.8296860066098; //S6K
    X[19]=0.0; //S6Kp
    X[20]=0.0; //S6p
    X[21]=99.9986712896423; //X
    X[22]=0.00132871035763352; //Xp
    X[23]=96.927052256569; //mTORC1
    X[24]=0.0; //mTORC1a
    X[25]=99.9045223943705; //mTORC2
    X[26]=0.0; //mTORC2a
