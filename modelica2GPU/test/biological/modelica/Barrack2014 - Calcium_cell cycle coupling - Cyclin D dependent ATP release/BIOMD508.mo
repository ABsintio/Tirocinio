...
model BIOMD508 "Barrack2014 - Calcium_cell cycle coupling - Cyclin D dependent ATP release"
...
equation
    ...
    der(atp) = scale * (vatp_s * (y - atp) * dcon * ip3con * ((ip3 - ip3min) / (krel + ip3)) - vdeg * atp / (kkdeg + atp));
    der(y) = scale * (alpha * (ymax - y) - dcon * ip3con * vatp_s * (y - atp) * ((ip3 - ip3min) / (krel + ip3)));
