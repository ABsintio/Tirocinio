...
#ifndef BIOMD442_FUNCTIONS_H
#define BIOMD442_FUNCTIONS_H

#include <iostream>

#define PRECISION double

__device__ PRECISION function_1(PRECISION V11,PRECISION X,PRECISION K11,PRECISION A,PRECISION species_7,PRECISION Ka){
    PRECISION y = 0.0;
    y=(V11 * (X * ((1.0 + (A * (species_7 / Ka))) / (((1.0 + (species_7 / Ka)) * K11) * (1.0 + (X / K11))))));
    return y;
}
__device__ PRECISION function_2(PRECISION V12,PRECISION X_P,PRECISION K12){
    PRECISION y = 0.0;
    y=(V12 * (X_P / (K12 + X_P)));
    return y;
}
