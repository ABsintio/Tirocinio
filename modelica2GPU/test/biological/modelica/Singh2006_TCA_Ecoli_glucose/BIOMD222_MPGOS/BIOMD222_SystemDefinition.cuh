...
#ifndef BIOMD222_PERTHREAD_SYSTEMDEFINITION_H
#define BIOMD222_PERTHREAD_SYSTEMDEFINITION_H
...
template<class Precision> __forceinline__ __device__ void PerThread_OdeFunction(
    ...
    F[1]=((sPAR[49] * ((((sPAR[30] * (X[7] / sPAR[14])) - (sPAR[41] * (X[1] / sPAR[2]))) / (1.0 + ((X[7] / sPAR[14]) + (X[1] / sPAR[2])))) + (((sPAR[44] * (X[10] / sPAR[22])) - (sPAR[33] * (X[1] / sPAR[4]))) / (1.0 + ((X[1] / sPAR[4]) + (X[10] / sPAR[22])))))) + (-0.188 * (sPAR[49] * (((sPAR[31] * (X[7] / sPAR[15])) - (sPAR[42] * (X[1] / sPAR[3]))) / (1.0 + ((X[7] / sPAR[15]) + (X[1] / sPAR[3])))))));
...
template<class Precision> __forceinline__ __device__ void PerThread_Initialization(
    ...
    sPAR[27]=91.2; //Vf_acn_ACN
    sPAR[28]=91.2; //Vf_cs_CS
    sPAR[29]=44.64; //Vf_fum_FUM
    sPAR[30]=14.72; //Vf_icd_ICD
    sPAR[31]=14.72; //Vf_icd_SYN
    sPAR[32]=1.9; //Vf_icl_ICL
    sPAR[33]=35.84; //Vf_kdh_KDH
    sPAR[34]=356.64; //Vf_mdh_MDH
    sPAR[35]=1.9; //Vf_ms_MS
    sPAR[36]=3.5; //Vf_scas_ScAS
    sPAR[37]=7.38; //Vf_sdh_SDH
    sPAR[38]=0.912; //Vr_acn_ACN
    sPAR[39]=0.912; //Vr_cs_CS
    sPAR[40]=37.2; //Vr_fum_FUM
    sPAR[41]=0.1472; //Vr_icd_ICD
    sPAR[42]=0.1472; //Vr_icd_SYN
    sPAR[43]=0.019; //Vr_icl_ICL
    sPAR[44]=0.3584; //Vr_kdh_KDH
    sPAR[45]=353.11; //Vr_mdh_MDH
    sPAR[46]=0.019; //Vr_ms_MS
    sPAR[47]=0.035; //Vr_scas_ScAS
    sPAR[48]=7.31; //Vr_sdh_SDH
    sPAR[49]=1.0; //cell
    X[0]=0.5; //aca
    X[1]=0.2; //akg
    X[2]=0.1; //biosyn
