...
void MoonLanding_function_initSample(DATA *data)
{
  ...
  data->modelData.samplesInfo[i].interval = $Pctr$PT;
...
int MoonLanding_checkForDiscreteChanges(DATA *data)
{
  ...
  if($Pctr$Pthrust != $P$PRE$Pctr$Pthrust)
  {
    infoStreamPrint(LOG_EVENTS_V, 0, "discrete var changed: ctr._thrust from %g to %g", $P$PRE$Pctr$Pthrust, $Pctr$Pthrust);
    needToIterate = 1;
  }
  if($Pctr$Phistory != $P$PRE$Pctr$Phistory)
  {
    infoStreamPrint(LOG_EVENTS_V, 0, "discrete var changed: ctr._history from %g to %g", $P$PRE$Pctr$Phistory, $Pctr$Phistory);
    needToIterate = 1;
  }
