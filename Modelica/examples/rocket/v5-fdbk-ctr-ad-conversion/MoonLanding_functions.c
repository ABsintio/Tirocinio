...
modelica_real omc_ECU_ad(threadData_t *threadData, modelica_real _x)
{
  ...
  modelica_real _HiVal;
  modelica_real _LoVal;
  modelica_real tmp1;
  ...
  else
  {
    ...
    {
      tmp1 = _Delta;
      if (tmp1 == 0) {throwStreamPrint(threadData, "Division by zero %s", "x / Delta");}
      _y = (_Delta) * (floor(0.5 + (_x) / tmp1));
    }
  }
