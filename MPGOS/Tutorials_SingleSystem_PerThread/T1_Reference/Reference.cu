...
// All necessary information for memory allocations
#define SOLVER RKCK45     // RK4, RKCK45
#define PRECISION double  // float, double
const int NT   = 23040; // NumberOfThreads during integration
const int SD   = 2;     // SystemDimension -> lunghezza del vettore X
...
int main()
{
	...
	/*
	Lists all CUDA capable devices in a machine and prints their most important properties to the screen
	*/
	ListCUDADevices();
	
	// Major version and minor version by the CC (GPU Compute Cabability), es. 3.5.
	// Mine is 7.5 => Major: 7, Minor: 5.
	/*
	If a GPU is required according to a specific compute capability, it is possibile to obtain a device 
	...
	*/
	...
	// Runna le NumberOfSimulationLaunches simulazioni
	for (int LaunchCounter=0; LaunchCounter<NumberOfSimulationLaunches; LaunchCounter++)
	{
		// 4. Fill the data structure of the SolverObject
		...
		// 5. Synchronise the data from the CPU (System Memory) to the GPU (Global Memory)
	...
	ScanDuffing.Print(DenseOutput, NT - 1);
